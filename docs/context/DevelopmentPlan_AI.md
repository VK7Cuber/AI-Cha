...
- Не задавать прямых вопросов по типу "Что вы хотите?" или "Какое у вас настроение?". Настроение     выводится из контекста ответов. (Однако вопрос по типу "Как дела?" не воспрещается, так как соответствует форме непринужденной беседы).
...
- Отправка всей истории диалога, а также списка доступных ингредиентов в GigaChat
...
**⚠️ Ключевой принцип:** AI старается вывести настроение из диалога с пользователем, не задавая прямых вопросов. Однако в некоторых случаях прямой вопрос "Как дела?" не воспрещается, так как соответствует форме непринуждённого диалога.
...
```
Проанализируй диалог с клиентом и **выведи** информацию из контекста следующую информацию из диалога:
{
  "mood": {
...
```
Ты - AI-бариста в чайном кафе. Твоя задача - придумывать персонализированные рецепты напитков для клиентов, подбирая рецепт на основе устного диалога, правил совместимости ингредиентов, твоего личного опыта и примеров готовых напитков из основного меню кафе. На основе профиля клиента и диалога создай уникальный рецепт напитка.
...
{полноценный диалог с пользователем (можно как список строк)}
...
6. Подробно опиши рецепт как последовательность действий, необходимую для приготовления
...
```
AI обязательно должен получитб все доступные ингредиенты для генерации подходящего напитка!
...
- Проверки: без кофеина, яркий, кислый, игровая подача
...
Прототип терминала использует экран 10.1" с разрешением 1024x600 пикселей. Все размеры должны быть крупными для комфортного использования на малом экране. Используйте относительные единицы (rem, em, %) для масштабируемости. Дизайн адаптивный.
